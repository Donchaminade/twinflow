...
const hops = [
  ...
  },
  {
    ...
    body: "Catalogue → SQLite local (~1 s). Tables fresh et tous les writes → PostgreSQL, puis invalidation immédiate du miroir.",
  },
];

export function LandingWorkflow() {
  return (
    <section id="workflow" className="mx-auto max-w-6xl px-4 py-16 md:py-24">
      <Reveal>
        <p className="eyebrow">Flux</p>
        <h2 className="font-heading mt-3 max-w-2xl text-3xl tracking-tight md:text-5xl">
          Client → sidecar → miroir / primary.
        </h2>
        <p className="mt-4 max-w-2xl text-muted-foreground">
          Un binaire et un YAML. Postgres reste la source de vérité. Le miroir
          n&apos;est pas un cache applicatif : c&apos;est une copie locale
          bornée, table par table.
        </p>
      </Reveal>

      <ol className="mt-12 grid gap-4 md:grid-cols-3">
        {hops.map((hop, i) => (
          <Reveal key={hop.title} as="li" delayMs={i * 90} className="glass relative p-6">
            <p className="font-mono text-xs text-primary">0{i + 1}</p>
            <h3 className="font-heading mt-3 text-2xl">{hop.title}</h3>
      ...
      <Reveal delayMs={120} className="mt-10 overflow-x-auto">
          ...
          <tbody className="text-foreground/90">
            <tr className="border-b border-border/50">
              ...
              <td className="py-3 text-muted-foreground">
                Primary si lag &gt; max_lag ou miroir down
              </td>
            </tr>
            <tr className="border-b border-border/50">
